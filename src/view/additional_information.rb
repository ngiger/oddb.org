    ...
    module AdditionalInformation
      ...
      def compositions(model, session = @session)
        ...
        parts = comps.collect { |comp|
          part = ""
          if (galform = comp.galenic_form && galform.respond_to?(lang.to_sym))
      ...
      def _fachinfo(model, css = "square infos")
        iksnr = ""
        if model.respond_to?(:fachinfo_active?) && model.fachinfo_active? # Registration
          iksnr = model.iksnr
        elsif (model.respond_to?(:iksnrs) && iksnrs = model.iksnrs) # FachinfoDocument
          iksnr = iksnrs.first
        end
        unless iksnr.empty?
      ...
      def ikscat(model, session = @session)
        ...
        if (gt = model.sl_generic_type) && [:generic, :original].index(gt)
      ...
      def limitation_link(sltxt, model = nil)
        ...
        if model.is_a?(ODDB::Package)
          ...
          reg = model.registration.iksnr
          seq = model.seqnr
          pack = if (packs = model.packages.values.select { |pac| pac.limitation_text } && !packs.empty?)
            packs.first.ikscd
          end

        end

        link.href = if reg && seq && pack
      ...
      def notify(model, session = @session)
        ...
        if model.is_a?(ODDB::Package)
        ...
        elsif model.is_a?(DRb::DRbObject) && model.respond_to?(:article_name) # ODDB::Migel::Item
          ...
          link.href = if (@session.state.respond_to?(:interval) && range = @session.state.interval)
      ...
      def patinfo(model, session = @session)
        if model.respond_to?(:has_patinfo?) && model.has_patinfo?
          href = nil
          klass = nil
          if (pdf_patinfo = model.pdf_patinfo && !pdf_patinfo&.empty?)
            klass = HtmlGrid::PopupLink
            href = @lookandfeel.resource_global(:pdf_patinfo, pdf_patinfo)
          elsif (model.patinfo && seqnr = model.seqnr)
            klass = HtmlGrid::Link
