...
in {
  ...
  scripts.kill_by_name.package = pkgs.fish;
  scripts.kill_by_name.exec = ''
    echo will kill processes with name $argv[1]
    export my_pid=(ps -eo pid,args | grep  $argv[1] | cut -f1 -d ' '|head -n 1)
    kill $my_pid
  '';
  ...
  scripts.kill_test_processes.exec = ''
    kill_by_name exportd
    kill_by_name fiparsed
    kill_by_name refdatad
    kill_by_name rackup
